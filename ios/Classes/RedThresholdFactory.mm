...
+ (void)processWhitPathType:(int)pathType pathString:(NSString *)pathString data:(FlutterStandardTypedData *)data minThresholdValue: (double)minThresholdValue result: (FlutterResult) result{
    ...
    switch (pathType) {
        case 1:
            result(redThresholdS(pathString, minThresholdValue));
            break;
        case 2:
...
FlutterStandardTypedData * redThresholdS(NSString * pathString, double minThresholdValue) {
    ...
    if (puedePasar) {
        ...
        cv::inRange(hsvImage, cv::Scalar(0, minThresholdValue, 100), cv::Scalar(10, 255, 255), mask1);
        cv::inRange(hsvImage, cv::Scalar(160, minThresholdValue, 100), cv::Scalar(180, 255, 255), mask2);
